#!/usr/bin/env python3
"""Main entry point for text processing tool."""
...
from .txt_reader import discover_txt_files
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Process text files with AI",
    ...
    parser.add_argument("--cost-only", action="store_true",
                       help="Estimate costs only using token counting API (no actual processing)")
    ...
    parser.add_argument("--batch-id", type=str,
                       help="Check status or fetch results for an existing batch")
    parser.add_argument("--list-batches", action="store_true",
                       help="List recent batches")
    parser.add_argument("--wait", action="store_true",
                       help="Wait for batch completion (use with batch mode)")
...
def validate_profile_selection(args):
    ...
    if not yaml_files:
        ...
        logger.error("Create a profile file first (e.g., haiku_4.5_0.3_REAL-TIME.yaml)")
        sys.exit(1)
    ...
    logger.error("Example: python3 -m src.main --profile haiku_4.5_0.3_REAL-TIME.yaml")
    sys.exit(1)
...
def discover_txts(args):
    """
    Discover TXT files based on arguments.

    Args:
        args: Parsed command line arguments

    Returns:
        Tuple of (list of text file paths, input directory path)
    """
    if args.input_dir:
        ...
        return discover_txt_files(input_path), input_path
    else:
...
def main():
    ...
    if args.list_profiles:
        # Special case: list profiles doesn't need config
        handler = CLIHandler({})
    ...
    # Handle batch operations (list, check status, fetch results)
    if handler.handle_batch_operations(args):
        return
    
    # Discover TXT files
    txt_files, input_dir = discover_txts(args)
    if not txt_files:
        logger.warning(f"No TXT files found in {args.input_dir or 'USER-FILES/04.INPUT/'}")
        return
    ...
    if args.cost_only or args.dry_run:
        # Dry run or cost-only mode: just estimate costs
        handler.handle_cost_estimation(txt_files, output_dir)
    elif handler.batch_mode:
        handler.handle_batch_submission(txt_files, args)
    else:
        handler.handle_realtime_processing(txt_files, output_dir, input_dir, False)
